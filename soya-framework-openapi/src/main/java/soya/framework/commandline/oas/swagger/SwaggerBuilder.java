package soya.framework.commandline.oas.swagger;
...
public class SwaggerBuilder {
    ...
    public static Swagger create(TaskExecutionContext context) {
        ...
        groups.forEach(e -> {
            if (context.groupDescription(e) != null) {
                TaskExecutionContext.GroupDescription groupDescription = context.groupDescription(e);
        ...
        for (String group : groups) {
            context.getCommands(group).forEach(e -> {
                buildPath(context.getTaskType(e), context, builder);
            });
        }
    ...
    private static void buildPath(Class<? extends TaskCallable> cls, TaskExecutionContext context, Swagger.SwaggerBuilder builder) {

        Command command = cls.getAnnotation(Command.class);
        TaskExecutionContext.GroupDescription groupDescription = context.groupDescription(command.group());
