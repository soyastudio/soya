package soya.framework.action.actions.javacodegen.rest;
...
public class RestApiGenerator extends JavaCodeBuilderAction {
    ...
    protected void printBody(CodeBuilder builder) {
        ...
        subTypes.forEach(c -> {
            ...
            if (command.group().equals(group)) {
                printCommandMethod((Class<? extends ActionCallable>) c, builder);
            }
        });
    }

    private void printCommandMethod(Class<? extends ActionCallable> cls, CodeBuilder builder) {
    ...
    class CommandMethodMapping {
        ...
        CommandMethodMapping(Class<? extends ActionCallable> cls) {
            ...
            this.fields = Arrays.asList(ActionClass.get(cls).getActionFields());
