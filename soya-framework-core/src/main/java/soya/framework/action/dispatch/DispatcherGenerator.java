...
public class DispatcherGenerator {
    ...
    public DispatcherGenerator(ActionContext context) {
    ...
    private Set<Class<?>> generate(ActionContext context) {
        Set<Class<?>> set = new HashSet<>();
        context.groups().forEach(e -> {
            List<Class<? extends ActionCallable>> commands = new ArrayList<>();
            context.getCommands(e).forEach(c -> {
                commands.add(context.getActionType(c));
            });
    ...
    private Class<?> create(String group, List<Class<? extends ActionCallable>> commands) throws
            CannotCompileException, IOException, NotFoundException {
        ClassPool cp = ClassPool.getDefault();
        CtClass superClass = cp.get(ActionDispatcher.class.getName());
        ...
        Annotation mappingAnnotation = new Annotation(ActionDispatcher.class.getName(), classFile.getConstPool());
...
*/
    ...
    static class CommandMethodMapping {
        ...
        CommandMethodMapping(Class<? extends ActionCallable> cls) {
            this.cls = cls;
            Command command = cls.getAnnotation(Command.class);

            this.command = command.name();
            this.fields = Arrays.asList(ActionParser.getOptionFields(cls));
