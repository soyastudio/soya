...
public class OverviewTask extends ReflectionTask<JsonElement> {
    private TaskExecutionContext context = TaskExecutionContext.getInstance();
    ...
    public JsonElement execute() throws Exception {
        if (filter == null || filter.isEmpty()) {
            ...
            Class<? extends TaskCallable> cls = context.getTaskType(TaskName.fromURI(key));
    ...
    private JsonArray groupInfo(String group) {
        ...
        context.getCommands(group).forEach(taskName -> {
            Class<? extends TaskCallable> cls = context.getTaskType(taskName);
    ...
    private JsonObject commandInfo(Class<? extends TaskCallable> cls) {
        Gson gson = new Gson();
