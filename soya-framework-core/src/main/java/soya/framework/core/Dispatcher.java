...
public abstract class Dispatcher {
    ...
    protected String _dispatch(String methodName, Object[] args) throws Exception {
        ...
        TaskCallable cmd = methodMappings.get(methodName).create(args);
        Future<?> future = TaskExecutionContext.getInstance().getExecutorService().submit(cmd);
        while (!future.isDone()) {
            Thread.sleep(100l);
        }
    ...
    protected Object _dispatch(URI uri) throws Exception {
        ...
        if (uri.getScheme().equals("class")) {
            ...
            cls = TaskExecutionContext.getInstance().getTaskType(TaskName.fromURI(key));
        ...
        if (cls != null) {
            ...
            TaskCallable cmd = TaskParser.create(cls, list.toArray(new String[list.size()]));
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (token.contains("{")) {
                ...
                try {
                    ...
                    paramPart = TaskExecutionContext.getInstance().getProperty(paramPart);
                }
    ...
    private static class CommandFactory<T extends TaskCallable> {
